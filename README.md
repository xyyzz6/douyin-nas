...
- ✅ **缩略图存在独立文件夹、重启不重抽**：生成的图落在 App 私有目录 `filesDir/thumbs/`（PC 版是 `data/thumbs/`），文件名是 `sha1(版本+视频路径).jpg`。App 重开、手机重启都不会重抽一遍；只有抽帧逻辑改了（版本号 +1）才会整套失效重做。「我的」页会显示「缩略图已存 N 张 · X MB」
