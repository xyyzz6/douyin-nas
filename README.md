...
```
       ...
       NAS 影迷
   NAS 短视频号：nas_xxxx
   [ 0 收藏 ] [ 2 点赞 ] [ 73 视频 ]   ← 三个数字都能点，点进去就是对应列表
   [ 数据源设置 ] [ 重新扫描 ]
     数据源：WebDAV http://…
     扫描时间：…
...
```
片源文件夹                                    2 个
📁 电影     /video/电影 · 34 个视频   [只刷它] [✕]
📁 剧集     /video/剧集 · 88 个视频   [只刷它] [✕]
─────────────────────────────────────
根目录 / video / 电影                    ← 面包屑，点任意一级跳回去
[↑ 上一级]  [↻]              [含子文件夹 ●]
[＋ 加入片源（128 个）]  [▶ 只刷它]
...
- 每个文件夹下面那行「里面 N 个视频」是**异步数的**：目录先列出来，数量随后补上，NAS 慢也不会干等
